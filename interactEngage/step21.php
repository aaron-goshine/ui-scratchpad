...
<script>
    $(document).ready(function(){
        ...
        var currentUser =
            {
                id: 1,
                avatarUrl: <?php if(isset($_GET['email'])) { echo '"http://www.gravatar.com/avatar/'.md5($_GET['email']).'"'; } else { ?>"http://placepic.me/starwars/100-100"<?php } ?>,
                name: <?php if(isset($_GET['email'])) { echo '"'.$_GET['email'].'"'; } else { ?>"Obi Wan"<?php } ?>
            };
        var existingComments = [
            {
                ...
                "comments": [
            <?php
                $query = "SELECT * FROM comments WHERE sectionId=1";
                $result = $con->query($query);

                while($row = $result->fetch_assoc()) { ?>

                    {
                        "authorAvatarUrl": <?php echo '"http://www.gravatar.com/avatar/'.md5($row['email']).'"';?>,
                        "authorName": "<?php echo $row['email']; ?>",
